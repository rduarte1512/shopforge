...
import { supabase, isSupabaseConfigured, getStoreEmailSettings, sendEmail, logEmail, getEmailTemplate, getAbandonedCartsNeedingRecovery, markAbandonmentEmailSent, isEmailUnsubscribed } from '@/lib/email';
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const cart of abandondedCarts) {
      ...
      const { data, error } = await sendEmail({ apiKey: settings.resend_api_key!, to: cart.email, subject, html, from: `${settings.sender_name} <${settings.sender_email || 'noreply@shopforge.dev'}>`, });
      await markAbandonmentEmailSent(cart.id);
      await logEmail({ store_id: storeId, email: cart.email, email_type: 'cart_recovery', subject, resend_message_id: data?.id, status: error ? 'failed' : 'sent', metadata: { abandonment_id: cart.id } });
      if (!error) sent++;
    }
