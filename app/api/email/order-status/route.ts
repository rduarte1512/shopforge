...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await sendEmail({ apiKey: settings.resend_api_key!, to: order.customer_email, subject, html, from: `${settings.sender_name} <${settings.sender_email || 'noreply@shopforge.dev'}>`, });
    await logEmail({ store_id: storeId, email: order.customer_email, email_type: 'order_status', subject, resend_message_id: data?.id, status: error ? 'failed' : 'sent', metadata: { order_id: orderId, new_status: order.status }, });
