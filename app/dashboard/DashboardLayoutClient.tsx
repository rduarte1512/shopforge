...
export default function DashboardLayoutClient({ children, user, initialStores }: DashboardLayoutClientProps) {
  ...
  const SidebarContent = () => (
    <div className="relative flex flex-col h-full py-5 px-4 overflow-hidden bg-slate-950 text-white">
          ...
          <AnimatePresence>
            {storeDropdownOpen && (
              <motion.div 
                ...
                className="absolute left-2 right-2 mt-2 bg-white text-slate-950 border border-slate-100 shadow-2xl rounded-2xl py-2 z-50 overflow-hidden"
              >
                {stores.map(store => (
                  <button
                    key={store.id}
                    ...
                    className={`flex items-center justify-between w-full px-4 py-3 text-[13px] transition-all ${
                      currentStore?.id === store.id 
                        ...
                        : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950 font-bold'
                    }`}
                  >
      ...
      <div className="relative z-10 mt-auto pt-5 px-2">
        <div className="p-4 rounded-[1.6rem] bg-white text-slate-950 shadow-2xl relative overflow-hidden group mb-4">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400">Seu Plano</span>
              <div className="p-1.5 rounded-lg bg-slate-950 text-white">
            ...
            <p className="text-[11px] text-slate-500 font-semibold mb-3">Funcionalidades desbloqueadas conforme o teu plano.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans antialiased text-text-primary">
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-[300px] bg-slate-950 border-r border-slate-900 z-40 shadow-2xl shadow-black/20">
        <SidebarContent />
      </aside>
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.aside 
              ...
              className="fixed inset-y-0 left-0 z-50 w-[300px] bg-slate-950 border-r border-slate-900 shadow-2xl lg:hidden"
            >
      ...
      <div className="flex-1 lg:pl-[300px] flex flex-col min-h-screen">
        <header className="bg-white/85 backdrop-blur-xl sticky top-0 z-30 h-18 flex items-center justify-between px-4 lg:px-8 border-b border-slate-200/70 shadow-sm shadow-black/[0.02]">
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block">
              ...
              <input 
                ...
                className="bg-slate-50 border border-slate-200 pl-11 pr-4 py-3 rounded-2xl text-[13px] w-[360px] focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all font-semibold shadow-inner shadow-black/[0.02]"
              />
            </div>
          </div>
          
          <div className="flex items-center gap-2 lg:gap-3">
            <div className="hidden sm:flex items-center gap-2 text-[11px] font-black bg-emerald-50 text-emerald-700 px-3 py-2 rounded-full border border-emerald-100">
            ...
            {currentStore && (
              <Link  
                ...
                target="_blank"
                className="hidden sm:flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-slate-950 text-white border border-slate-900 text-[13px] font-black hover:bg-slate-800 transition-all shadow-lg shadow-black/10"
              >
            ...
            <button className="flex items-center gap-2 p-1.5 pr-3 rounded-full hover:bg-slate-100 transition-all border border-transparent hover:border-slate-200">
              ...
              <div className="hidden md:flex flex-col text-left leading-none">
                <span className="text-[13px] font-black text-slate-800">{user?.firstName || 'Conta'}</span>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{subscriptionTier}</span>
              </div>
            </button>
          </div>
        </header>
        
        <main className="flex-1 p-4 lg:p-8 bg-[radial-gradient(circle_at_top_right,_rgba(16,185,129,0.08),_transparent_32%),linear-gradient(180deg,#f8fafc_0%,#ffffff_100%)]">
          <motion.div 
            key={pathname}
