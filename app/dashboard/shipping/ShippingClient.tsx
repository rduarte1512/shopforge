...
export default function ShippingClient({ initialMethods, selectedStoreId }: ShippingClientProps) {
  const [shippingMethods, setShippingMethods] = useState<any[]>(initialMethods);
  ...
  const refreshMethods = async () => {
    ...
    const data = await getShippingMethodsAction(selectedStoreId);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <div key={method.id} className={`bg-white rounded-xl border-2 transition-all p-6 relative ${method.active ? 'border-[var(--color-border)] shadow-[var(--shadow-card)]' : 'border-dashed border-gray-200 grayscale opacity-70'}`}>
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => openModal(method)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-primary transition-all">
            ...
            <div className="space-y-3">
              ...
              {method.min_order_for_free && (
                <div className="flex items-center gap-3 text-[13px] font-medium text-emerald-600">
        ...
        {shippingMethods.length === 0 && (
          <div className="col-span-full py-20 bg-slate-50 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-center px-6">
            ...
            <button onClick={() => openModal()} className="mt-6 text-[13px] font-bold text-primary hover:underline">Adicionar a minha primeira opção</button>
          </div>
        )}
      </div>
           ...
           <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                 <div>
                    ...
                    <input required type="text" placeholder="EX: Envio Standard" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-[var(--color-border)] rounded-lg focus:outline-none" />
                 </div>
                 <div>
                    <label className="block text-xs font-bold text-[var(--color-text-muted)] uppercase mb-2">Descrição Curta</label>
                    <textarea required rows={2} placeholder="EX: Entrega em mão." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-[var(--color-border)] rounded-lg focus:outline-none" />
                 </div>
                 
                 <div className="grid grid-cols-2 gap-5">
                   <div>
                     <label className="block text-xs font-bold text-[var(--color-text-muted)] uppercase mb-2">Custo (€)</label>
                     <input required type="number" step="0.01" min="0" placeholder="0.00" value={formData.cost} onChange={e => setFormData({...formData, cost: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-[var(--color-border)] rounded-lg focus:outline-none" />
                   </div>
                   <div>
                     <label className="block text-xs font-bold text-[var(--color-text-muted)] uppercase mb-2">Tempo de Entrega</label>
                     <input required type="text" placeholder="EX: 2-3 dias úteis" value={formData.deliveryTime} onChange={e => setFormData({...formData, deliveryTime: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-[var(--color-border)] rounded-lg focus:outline-none" />
                   </div>
                 </div>

                 <div>
                    <label className="block text-xs font-bold text-[var(--color-text-muted)] uppercase mb-2">Grátis a partir de (€)</label>
                    <input type="number" step="0.01" min="0" placeholder="Opcional" value={formData.minOrderForFree} onChange={e => setFormData({...formData, minOrderForFree: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-[var(--color-border)] rounded-lg focus:outline-none" />
                 </div>
