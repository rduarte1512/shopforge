...
import { recoverAbandonedCartAction } from '@/lib/abandoned-cart-actions';
...
function getStoredStoreCustomer(storeId?: string) {
...
function getCartSessionId(storeKey: string) {
  try {
    return localStorage.getItem(`shopforge-cart-session-${storeKey}`);
  } catch {
    return null;
  }
}
...
export default function CartPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const loggedCustomer = getStoredStoreCustomer(storefrontData.store.id);
        if (loggedCustomer) {
          setCustomer(loggedCustomer);
          setFormData({ name: loggedCustomer.name || '', email: loggedCustomer.email || '' });
        }
  ...
  const subtotal = useMemo(() => cartItems.reduce((sum, item) => sum + (Number(item.product?.price || 0) * item.quantity), 0), [cartItems]);
  ...
  const discountAmount = useMemo(() => {
    ...
    if (appliedCoupon.discount_type === 'percentage') return subtotal * (Number(appliedCoupon.discount_value) / 100);
    return Number(appliedCoupon.discount_value) || 0;
  }, [appliedCoupon, subtotal]);

  const total = Math.max(0, subtotal + shippingCost - discountAmount);
  const checkoutName = customer && useAccount ? customer.name : formData.name;
  const checkoutEmail = customer && useAccount ? customer.email : formData.email;
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (refCode && store) {
        ...
        if (linkData) affiliateLinkId = linkData.id;
      }
      ...
      const orderResult = await createStorefrontOrderAction({
        store_id: store.id,
      ...
      if (!orderResult?.success) throw new Error(orderResult?.error || 'Não foi possível guardar a encomenda.');

      const sessionId = getCartSessionId(store.id);
      if (sessionId) recoverAbandonedCartAction(store.id, sessionId).catch(console.error);

      clearCart();
      setPaymentInfo({ name: selectedPayment.name, instructions: selectedPayment.instructions || '' });
      setSuccess(true);
    } catch (error: any) {
    ...
    return (
      <div className="min-h-screen bg-[#f7f3ed] px-6 py-24">
        <div className="max-w-3xl mx-auto text-center animate-in fade-in zoom-in-95 duration-500">
          ...
          <p className="text-slate-500 mb-8 text-lg max-w-md mx-auto font-medium">A sua encomenda foi registada com sucesso.</p>
          {paymentInfo?.instructions && (
            <div className="bg-white rounded-[2rem] p-7 mb-10 text-left max-w-xl mx-auto border border-black/5 shadow-2xl shadow-black/10">
              ...
              <h3 className="font-black text-lg text-slate-950 mb-3">{paymentInfo.name}</h3>
              <p className="text-slate-600 leading-relaxed text-sm font-medium">{paymentInfo.instructions}</p>
            </div>
          )}
          <Link href={`/s/${store.domain}`} className="inline-flex items-center justify-center gap-2 px-10 py-4 rounded-2xl text-white font-black shadow-xl shadow-black/10 bg-slate-950">
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-8 items-end">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/10 shadow-2xl text-xs font-black uppercase tracking-[0.24em] text-white/70 mb-5">
                <ShieldCheck className="w-4 h-4" /> Checkout seguro e premium
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 rounded-[1.7rem] bg-white/8 border border-white/10 p-2 backdrop-blur-xl">
              {['Carrinho', 'Entrega', 'Pagamento'].map((step, index) => (
                <div key={step} className={`px-3 py-4 rounded-2xl text-center text-[11px] font-black uppercase tracking-[0.18em] ${index === 0 ? 'bg-white text-slate-950' : 'text-white/45'}`}>{step}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-6 -mt-20 pb-20">
        {cartItems.length === 0 ? (
          <div className="text-center py-28 bg-white rounded-[2.5rem] border border-black/5 shadow-2xl shadow-black/10">
            <Wallet className="w-12 h-12 text-slate-300 mx-auto mb-6" />
            <p className="text-2xl font-black text-slate-950 mb-3">O seu carrinho está vazio.</p>
            <Link href={`/s/${store.domain}`} className="inline-block px-10 py-4 rounded-2xl text-white font-black shadow-xl shadow-black/10 bg-slate-950">Começar a Comprar</Link>
          </div>
        ) : (
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-white border border-black/5 rounded-[2.5rem] shadow-2xl shadow-black/10 overflow-hidden">
                ...
                <div className="p-4 md:p-6 space-y-4">
                  {cartItems.map((item) => (
                    <div key={`${item.productId}-${item.variantId || 'base'}`} className="group rounded-[2rem] bg-[#fbfaf7] border border-black/5 p-4 md:p-5 flex gap-4 md:gap-6 hover:bg-white transition-all">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                        <div className="flex justify-between gap-4 items-start">
                          <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.28em] mb-2">{item.product?.category || 'Produto premium'}</p>
                            <h3 className="font-black text-xl md:text-2xl leading-tight text-slate-950">{item.product?.name}</h3>
                            <span className="inline-flex mt-4 rounded-full bg-white border border-black/5 px-3 py-1 text-xs font-black text-slate-600">Quantidade: {item.quantity}</span>
                          </div>
                          <p className="font-black text-xl md:text-2xl whitespace-nowrap text-slate-950">{formatPrice(Number(item.product?.price || 0) * item.quantity)}</p>
                        </div>
                        <button onClick={() => removeItem(item.productId, item.variantId)} className="text-xs font-black text-slate-300 hover:text-rose-500 flex items-center gap-2 transition-colors w-fit mt-6 uppercase tracking-[0.18em]">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div className="bg-white border border-black/5 rounded-[2.5rem] shadow-2xl shadow-black/10 p-6 md:p-7">
                  <div className="flex items-center gap-3 mb-6"><div className="w-12 h-12 rounded-2xl bg-slate-950 text-white flex items-center justify-center"><Truck className="w-5 h-5" /></div><div><p className="text-[10px] uppercase tracking-[0.28em] text-slate-400 font-black">Entrega</p><h2 className="text-xl font-black text-slate-950">Opções de envio</h2></div></div>
                  <div className="space-y-3">
                    {shippingMethods.map(method => {
                      const selected = selectedShipping?.id === method.id;
                      return <button key={method.id} onClick={() => setSelectedShippingId(method.id)} className={`w-full p-4 rounded-3xl border text-left transition-all ${selected ? 'bg-slate-950 text-white border-slate-950 shadow-xl shadow-black/10' : 'border-black/5 bg-[#fbfaf7] hover:bg-white text-slate-950'}`}><div className="flex justify-between items-center mb-1 gap-3"><p className="font-black">{method.name}</p><p className="font-black">{method.min_order_for_free && subtotal >= Number(method.min_order_for_free) ? 'Grátis' : formatPrice(Number(method.cost))}</p></div><p className={`text-xs ${selected ? 'text-white/60' : 'text-slate-500'} font-semibold leading-relaxed`}>{method.delivery_time} • {method.description}</p></button>;
                    })}
                  </div>
                </div>

                <div className="bg-white border border-black/5 rounded-[2.5rem] shadow-2xl shadow-black/10 p-6 md:p-7">
                  <div className="flex items-center gap-3 mb-6"><div className="w-12 h-12 rounded-2xl bg-slate-950 text-white flex items-center justify-center"><Wallet className="w-5 h-5" /></div><div><p className="text-[10px] uppercase tracking-[0.28em] text-slate-400 font-black">Pagamento</p><h2 className="text-xl font-black text-slate-950">Métodos disponíveis</h2></div></div>
                  <div className="space-y-3">
                    {paymentMethods.map((method: StorefrontPaymentMethod) => {
                      const selected = selectedPayment?.id === method.id;
                      return <button key={method.id} onClick={() => setSelectedPaymentId(method.id)} className={`w-full relative p-4 rounded-3xl border text-left transition-all overflow-hidden ${selected ? 'bg-slate-950 text-white border-slate-950 shadow-xl shadow-black/10' : 'border-black/5 bg-[#fbfaf7] hover:bg-white text-slate-950'}`}><div className="flex items-start gap-3"><div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${selected ? 'bg-white/15' : 'bg-white border border-black/5 shadow-sm'}`}><PaymentIcon method={method} selected={selected} /></div><div><p className="font-black">{method.name}</p><p className={`text-xs ${selected ? 'text-white/60' : 'text-slate-500'} font-semibold leading-relaxed`}>{method.description}</p></div></div></button>;
                    })}
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-24 rounded-[2.5rem] bg-[#080808] text-white shadow-2xl shadow-black/25 overflow-hidden border border-white/10">
                <div className="relative z-10 p-6 md:p-7">
                  <div className="flex items-center justify-between mb-8"><div><p className="text-[10px] uppercase tracking-[0.3em] text-white/35 font-black mb-2">Resumo</p><h2 className="text-2xl font-black tracking-tight">Finalizar compra</h2></div><div className="w-12 h-12 rounded-2xl bg-white text-slate-950 flex items-center justify-center"><LockKeyhole className="w-5 h-5" /></div></div>

                  <div className="mb-7">
                    <p className="text-[10px] font-black text-white/35 uppercase tracking-[0.28em] mb-3">Cupão de desconto</p>
                    <div className="flex gap-2"><div className="relative flex-1"><Ticket className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-white/35" /><input type="text" placeholder="Código" value={couponCode} onChange={e => setCouponCode(e.target.value)} className="w-full pl-10 pr-4 py-4 rounded-2xl bg-white/8 border border-white/10 focus:outline-none focus:border-white/35 font-mono font-black text-sm text-white placeholder:text-white/25" /></div><button onClick={handleApplyCoupon} className="px-5 py-4 bg-white text-slate-950 rounded-2xl font-black text-sm hover:bg-white/90 transition-colors">Aplicar</button></div>
                  ...
                  <div className="space-y-4 mb-7 rounded-[2rem] bg-white/[0.06] p-5 border border-white/10">
                    <div className="flex justify-between text-sm font-bold text-white/55"><span>Subtotal</span><span className="text-white">{formatPrice(subtotal)}</span></div>
                    <div className="flex justify-between text-sm font-bold text-white/55"><span>Envio</span><span className="text-white">{shippingCost === 0 ? 'Grátis' : formatPrice(shippingCost)}</span></div>
                    {selectedPayment && <div className="flex justify-between text-sm font-bold text-white/55"><span>Pagamento</span><span className="text-white text-right">{selectedPayment.name}</span></div>}
                    {discountAmount > 0 && <div className="flex justify-between text-sm font-black text-emerald-300"><span>Desconto</span><span>- {formatPrice(discountAmount)}</span></div>}
                    <div className="pt-5 border-t border-white/10 flex justify-between items-end"><span className="font-black text-white/70">Total</span><span className="font-black text-4xl tracking-tighter text-white">{formatPrice(total)}</span></div>
                  </div>

                  <form onSubmit={handleCheckout} className="space-y-4">
                    <p className="text-[10px] font-black text-white/35 uppercase tracking-[0.28em]">Cliente</p>
                    {accountsEnabled && customer && useAccount ? (
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2"><div className="rounded-2xl bg-white/[0.06] border border-white/10 p-3 flex items-center gap-2 text-[11px] font-bold text-white/55"><BadgeCheck className="w-4 h-4 text-emerald-300" /> Seguro</div><div className="rounded-2xl bg-white/[0.06] border border-white/10 p-3 flex items-center gap-2 text-[11px] font-bold text-white/55"><PackageCheck className="w-4 h-4 text-emerald-300" /> Protegido</div></div>
                  </form>
                </div>
              </div>
            </div>
